...
p=pathlib.Path('D:/Code/ExtremeEngine-CppSLJIT/engine/modules/gui/core/tests/text/text_shaping_contract_tests.cpp')
...
def tr(s):
 ...
 s=s.replace('style.OpenTypeFeatures.Add({','style.OpenTypeFeatures.Add(new TextOpenTypeFeature {')
 ...
 s=s.replace('expect_near(', 'Check.Near(')
 s=s.replace('std.round(', 'Round(')
 s=re.sub(r'(?<=[=\(,])\s*&(?=\w)', '', s)
 s=re.sub(r'(?m)^(\s*)auto (\w+) = \[&\]\((.*?)\)\s*\{', r'\1var \2 = (\3) => {', s, flags=re.S)
 s=s.replace('TextGlyph*','TextGlyph?')
 s=s.replace('.GlyphIndex(','.GlyphIndex(')
 s=s.replace('kTextDefaultJustificationFlags','TextDefaults.JustificationFlags')
 ...
 return s
# Helpers outside the original anonymous namespace.
helper=clean[clean.index('bool same_glyph_identity'):clean.index('} \n\nSKR_TEST_CASE') if '} \n\nSKR_TEST_CASE' in clean else clean.index('SKR_TEST_CASE')]

helper=tr(helper)
helper=helper.replace('TextGlyph* source_glyph_at','TextGlyph? source_glyph_at').replace('return &glyph;','return glyph;')
helper=re.sub(r'(?m)^(bool|Rectf|float|void) (\w+)\(',r'private static \1 \2(',helper)
helper=helper.replace('TextGlyph? source_glyph_at','private static TextGlyph? source_glyph_at')
extra='''using static SkrGui.Tests.TextContractFixture;
...
private static void Near(float a,float b)=>Check.Near(b,a,Math.Max(Math.Abs(b)*0.00001,0.00001));
...
for name,fields in structs:
 translated=tr(fields)
 translated=re.sub(r'(?m)^(\s*)(ETextDirectionMode|Utf8StringView)\s+',r'\1public \2 ',translated)
 translated=re.sub(r'(public (?:ETextDirectionMode|Utf8StringView) )(\w+)',lambda m:m[1]+pascal(m[2]),translated)
 extra+='private sealed class '+name+' { '+translated+' }\n'
output=extra+helper
cases=[]
for i,m in enumerate(re.finditer(r'SKR_TEST_CASE\((STRTOKEN\d+TOKEN)\)\s*\{',clean)):
 ...
 cases.append({'source':'tests/text/text_shaping_contract_tests.cpp','case':name,'target':f'TextShapingContractTests.SourceCase{i+1:02}'})
output+='}\n'
(root/'tests/SkrGui.Core.Tests/Text/TextShapingContractTests.cs').write_text(output,encoding='utf-8')
(root/'migration/text-shaping-test-map.json').write_text(json.dumps(cases,indent=2),encoding='utf-8')
