...
p=pathlib.Path('D:/Code/ExtremeEngine-CppSLJIT/engine/modules/gui/core/tests/text/text_layout_contract_tests.cpp')
...
for i,m in enumerate(re.finditer(r'SKR_TEST_CASE\((STRTOKEN\d+TOKEN)\)\s*\{',clean)):
 ...
 cases.append({'source':'tests/text/text_layout_contract_tests.cpp','case':name,'target':f'TextLayoutContractTests.SourceCase{i+1:02}'})
output+='}\n'
(root/'tests/SkrGui.Core.Tests/Text/TextLayoutContractTests.cs').write_text(output,encoding='utf-8')
(root/'migration/text-layout-test-map.json').write_text(json.dumps(cases,indent=2),encoding='utf-8')
