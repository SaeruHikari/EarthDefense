...
p=pathlib.Path('D:/Code/ExtremeEngine-CppSLJIT/engine/modules/gui/core/tests/text/text_raster_contract_tests.cpp')
source=p.read_text(encoding='utf-8')
cut=source.rfind('    struct RasterRoute'); source=source[:cut]+source[cut:].replace('RasterRoute','BitmapRasterRoute')
...
def tr(s):
 ...
 s=s.replace('uint8_t','byte').replace('Optional<Offsetf>','Offsetf?')
 s=re.sub(r'std\.Numeric_limits<(\w+)>\.Max\(\)',r'\1.MaxValue',s)
 s=s.replace('std.Lround(', 'Round(').replace('std.Round(', 'Round(')
 s=s.replace('expect_near(', 'Check.Near(').replace('kFloatEpsilon', '0.0001f')
 s=re.sub(r'auto (\w+) = \[&?\]\((.*?)\)\s*\{',r'var \1 = (\2) => {',s,flags=re.S)
 s=re.sub(r'auto \[([^]]+)\]\s*=',r'var (\1) =',s)
 s=s.replace('std.Make_pair(', 'ValueTuple.Create(')
 s=s.replace('std.Move(', '(')
 s=s.replace('npos_of<uint>','uint.MaxValue')
 s=re.sub(r'SKR_TEST_SUBCASE\(STRTOKEN\d+TOKEN\)', '',s)
...
for i,m in enumerate(re.finditer(r'SKR_TEST_CASE\((STRTOKEN\d+TOKEN)\)\s*\{',clean)):
 ...
 cases.append({'source':'tests/text/text_raster_contract_tests.cpp','case':name,'target':f'TextRasterContractTests.SourceCase{i+1:02}'})
output+='}\n'
(root/'tests/SkrGui.Core.Tests/Text/TextRasterContractTests.cs').write_text(output,encoding='utf-8')
(root/'migration/text-raster-test-map.json').write_text(json.dumps(cases,indent=2),encoding='utf-8')
