...
internal static class EarthScaleCombat
{
    ...
    {
        _checks=_failures=0;
        Near(WorldScale.EarthRadius,16,"actual shared Earth radius");Near(CombatScale.EarthRadius,16,"combat aliases shared Earth");Near(CombatScale.EarthCollisionRadius,16.07,"collision preserves .07 altitude");Near(CombatScale.CloseAssault,16.35,"bombard preserves .35 altitude");Near(CombatScale.DroneAltitude,.7,"drone altitude unchanged");Near(CombatScale.KineticRange,1.65,"kinetic range unchanged");Near(CombatScale.WeaponRange,4.5,"other weapon range unchanged");Near(CombatScale.PlanetPixelRadius,208,"model/effect conversion unchanged");
    ...
    private static void RunSpawningAndCombat()
    {
        ...
        var ground=battle.SpawnEnemy("scout",new(){["wave"]=1L,["stage"]=0,["role"]="claw"},Vector3.Back*16.35f)!;ground["phase"]="ground_attack";Check((bool)Invoke(battle,"CanBombardEarth",ground)!,"enemy enters correctly translated bombard altitude");double shield=game.EarthHp;Invoke(battle,"FireHostile",ground,2d,false);battle.UpdateShots(.3);Near(game.EarthHp,shield-2,"real planetary shot hits new collision radius");
