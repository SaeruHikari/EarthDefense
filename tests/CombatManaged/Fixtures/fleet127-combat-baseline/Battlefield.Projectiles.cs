...
public sealed partial class Battlefield
{
    private sealed class ColliderGrid
    {
        ...
        private readonly Dictionary<Vector3I, List<int>> _cells = new(); private double _maximum;
    ...
    public bool ResolveFriendlySegment(DataMap shot, Vector3 previous, Vector3 current)
    {
        ...
        for (int j = 0; j < 4; j++)
        {
            ...
            foreach (int index in candidates)
            {
                ...
                if (!bounds.Intersects(c.Bounds.Grow((float)proximity)) || C.N(c.Actor, "hp") <= 0 || visited.Any(v => DataMap.Integer(v) == C.L(c.Actor, "uid")))
                    continue;
