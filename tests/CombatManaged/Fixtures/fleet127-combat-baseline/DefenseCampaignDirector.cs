...
public sealed class DefenseCampaignDirector
{
    ...
    private void UpdateEarthSchedule(double dt)
    {
        ...
        var plan = new DataMap { ["id"] = $"continuing:{next}", ["wave"] = next, ["carrier_count"] = count, ["aircraft_per_carrier"] = settings.I("earth_escort_count", 12), ["duration"] = Game.CombatSettings.N("enemy_wave_duration", 45), ["unit_health"] = _baselineHealth * (1 + settings.N("earth_health_growth", .1) * (next - 1)), ["damage_multiplier"] = 1 + settings.N("earth_damage_growth", .06) * (next - 1), ["volley_count"] = 1, ["hangar_interval"] = 1.8, ["defense_stage"] = stage, ["frontier_radius"] = Game.CombatSettings.N("frontier_radius_" + stage, CombatScale.DefaultFrontierRadius(stage)) };
        plan["carrier_health"] = plan.N("unit_health") * 3;
