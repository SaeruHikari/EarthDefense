...
public static class EnemyArmor
{
    ...
    public static double Multiplier(DataMap enemy, string family, DataMap? context = null, double clock = 0)
    {
        ...
        double result = (layer, family) switch
        {
            ("light", "kinetic") => 1.6,
            ("light", "explosive" or "beam") => .65,
            ("heavy", "kinetic") => .2,
            ("heavy", "explosive") => 1.65,
            ("heavy", "beam") => .65,
            ("energy", "kinetic") => 0,
            ("energy", "explosive") => .2,
            ("energy", "beam") => 1.8,
            ("light" or "heavy" or "energy", _) => 0,
            _ => 1
        };
        if (result <= 0)
            return 0;
        ...
        if (layer == "energy" && family == "beam")
        {
            ...
            if (C.B(tech, "L_G1"))
                result *= C.Packet(context, "laser_energy_multiplier", 2);
        }
