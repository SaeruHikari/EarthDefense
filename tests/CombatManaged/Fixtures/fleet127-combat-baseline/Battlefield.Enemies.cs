...
public sealed partial class Battlefield
{
    ...
    private readonly Dictionary<Vector3I, List<DataMap>> _defenderBuckets = new();
    ...
    public bool UseEmp()
    {
        ...
        {
            e["stagger_until"] = Clock + (C.Large(e) ? .3 : .6);
    ...
    public double EnemyFireCooldown(string kind, string targetKind = "drone") => kind switch { "scout" => targetKind == "drone" ? 1.05 : 1.55, "cruiser" => targetKind == "drone" ? 1.25 : 1.55, "small_boss" => targetKind == "drone" ? 1.35 : 1.75, "boss" => 1.25, _ => 1.25 };
    ...
    private void EnsureDefenders()
    {
        if (_defendersValid)
            return;
        _defenderBuckets.Clear();
        foreach (var d in Drones)
        {
            if (C.N(d, "hp") <= 0 || C.N(d, "launch_age") < CombatScale.LaunchDuration || C.S(d, "state") is "launching" or "landing" or "repairing")
                continue;
            var n = GetDroneWorldPosition(d).Normalized();
            var key = new Vector3I((int)Math.Floor(n.X / .32), (int)Math.Floor(n.Y / .32), (int)Math.Floor(n.Z / .32));
            if (!_defenderBuckets.TryGetValue(key, out var list))
                _defenderBuckets[key] = list = new();
            list.Add(d);
        }
    ...
    private DataMap QueryDefender(Vector3 origin, bool requireRange, bool specialists = false, bool large = false)
    {
        ...
        double limit = Math.Min(Math.PI, 2.4 / CombatScale.EarthRadius), cos = Math.Cos(limit), chord = 2 * Math.Sin(limit * .5) + .00001, bestDistance = double.PositiveInfinity;
        int priorityBest = 99, indexBest = int.MaxValue;
        DataMap best = C.Empty;
        int minx = (int)Math.Floor(Math.Max(-1, normal.X - chord) / .32), maxx = (int)Math.Floor(Math.Min(1, normal.X + chord) / .32), miny = (int)Math.Floor(Math.Max(-1, normal.Y - chord) / .32), maxy = (int)Math.Floor(Math.Min(1, normal.Y + chord) / .32), minz = (int)Math.Floor(Math.Max(-1, normal.Z - chord) / .32), maxz = (int)Math.Floor(Math.Min(1, normal.Z + chord) / .32);
        for (int x = minx; x <= maxx; x++)
            for (int y = miny; y <= maxy; y++)
                for (int z = minz; z <= maxz; z++)
                {
                    if (!_defenderBuckets.TryGetValue(new(x, y, z), out var list))
                        continue;
                    foreach (var d in list)
                    {
                        var pos = GetDroneWorldPosition(d);
                        if (pos.Normalized().Dot(normal) < cos || Home(d).Dot(normal) < GetProfile(d).Cosine || C.N(d, "hp") <= 0)
                            continue;
                        int p = specialists ? (C.S(d, "kind") == "interceptor" ? 1 : 0) : (C.S(d, "kind") == "interceptor" ? 0 : 1);
                        if (p > priorityBest)
                            continue;
                        double distance = pos.DistanceSquaredTo(origin), range = GetProfile(d).Range * (large && HasTech("C_N1") ? C.N(_globalWeapons, "large_target_range_multiplier", 1.2) : 1);
                        if (requireRange && (distance > range * range || !CombatGeometry.HasLineOfSight(pos, origin)))
                            continue;
                        int index = C.I(d, "_actor_order");
                        if (p < priorityBest || distance < bestDistance || distance == bestDistance && index < indexBest)
                        {
                            best = d;
                            priorityBest = p;
    ...
    private void FireAtDrone(DataMap e, DataMap d, bool interceptable = false)
    {
        ...
        double speed = GetEnemyProjectileSpeed() * (interceptable ? .6 : 1), time = CombatGeometry.InterceptTime(position - origin, velocity, speed, 5);
    ...
    private bool UpdateEnemySkill(DataMap e, double dt)
    {
        ...
        if (role is "weaver" or "jammer")
        {
            ...
            {
                e["skill_clock"] = role == "weaver" ? .5 : 3d;
                if (role == "weaver")
                    WeaveEnergy(e);
                else
                {
    ...
    private void WeaveEnergy(DataMap e)
    {
        ...
        foreach (var target in Neighbors(C.V(e, "space_position"), 2, new long[] { C.L(e, "uid") }, 6))
        {
            if (C.B(target, "shield_broken") || Clock < C.N(target, "energy_recovery_blocked_until"))
                continue;
            if (C.N(target, "energy_max_hp") <= 0)
            {
                if (C.B(target, "energy_granted") || links.Count >= 6)
                    continue;
                target["energy_granted"] = true;
                target["energy_owner_uid"] = C.L(e, "uid");
                target["energy_max_hp"] = C.N(target, "max_hp") * .2;
                target["energy_hp"] = target["energy_max_hp"];
                target["energy_recovery_budget"] = C.N(target, "energy_max_hp") * .5;
                links.Add(C.L(target, "uid"));
            }
            else
            {
                double budget = C.N(target, "energy_recovery_budget", C.N(target, "energy_max_hp") * .5), amount = Math.Min(budget, Math.Min(C.N(target, "energy_max_hp") - C.N(target, "energy_hp"), C.N(target, "energy_max_hp") * .05));
