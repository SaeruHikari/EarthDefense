...
public sealed partial class Battlefield
{
    ...
    public Vector3 GetDroneWorldPosition(DataMap d) => WorldCache.TryGetValue(C.L(d, "uid"), out var position) ? position : RawDronePosition(d);
    private void CachePosition(DataMap d) => WorldCache[C.L(d, "uid")] = RawDronePosition(d);
    public DataMap? GetDroneByUid(long uid) => _droneById.TryGetValue(uid, out var d) && C.N(d, "hp") > 0 ? d : Drones.Find(x => C.L(x, "uid") == uid && C.N(x, "hp") > 0);
    ...
    public Vector3 SurfaceToSpace(Vector3 normal, double altitude = CombatScale.DroneAltitude) => Surface?.SurfaceToSpace(normal, altitude) ?? C.Scale(normal.Normalized(), CombatScale.EarthRadius + altitude);
    public Vector3 SpaceToSurface(Vector3 position) => Surface?.SpaceToSurface(position) ?? position.Normalized();
    private Vector3 Home(DataMap d) => Factories.TryGetValue(C.L(d, "factory_site_id"), out var f) ? C.V(f, "_world_normal", SurfaceToSpace(C.V(C.M(f, "site"), "normal"), 0).Normalized()) : GetDroneWorldPosition(d).Normalized();
    ...
    private double TargetRangeForProfile(Profile profile, bool largeTarget) => profile.Range * (largeTarget && HasTech("C_N1") ? C.N(_globalWeapons, "large_target_range_multiplier", 1.2) : 1);
