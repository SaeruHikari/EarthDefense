...
public sealed partial class Battlefield
{
    ...
    private static bool ValidatePerkShot(DataMap s)
    {
        ...
        if (s.ContainsKey("source"))
        {
            ...
            if (source.ContainsKey("primary") && source["primary"] is not bool)
                return false;
            if (source.ContainsKey("tech_abilities") && (source["tech_abilities"] is not DataMap tech || tech.Count > 48 || tech.Values.Any(v => v is not bool)))
                return false;
    ...
    private static bool ValidateActors(DataMap d, double storedEarthRadius)
    {
        ...
        foreach (var obj in d.List("_beams"))
            if (obj is not DataMap b || !CombatSnapshotCodec.HasFields(b, "from_space:v3 to_space:v3 life:number max_life:number color:color"))
                return false;
