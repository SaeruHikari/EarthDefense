...
public sealed partial class DefenseState
{
    ...
    public DataMap TechEffects()
    {
        ...
        double radius = Tech.N("mothership_assault") > 0 ? 12 + WorldScale.EarthRadiusDelta : 0;
        for (int stage = 1; stage <= 3; stage++)
            if (Tech.N("frontier_range_" + stage) > 0)
                radius = new double[] { 50, 84, 128 }[stage - 1] + WorldScale.EarthRadiusDelta;
    ...
    public List<DataMap> GraphNodes(string focusId = "")
    {
        ...
        foreach (var source in DeepTechnology.Nodes)
        {
            ...
            int stage = row.Map("unlock").I("defense_stage");
            string family = row.S("branch") switch
            {
                "K" => "interceptor",
                "M" => "missile",
                "L" => "laser",
            ...
            row["visible"] = HasResearch(row.S("id")) || ((stage <= DefenseReachStage || (row.S("size") == "large" && stage <= DefenseReachStage + 1)) && (family.Length == 0 || WeaponUnlocked(family) || row.S("id") is "M_N1" or "L_N1"));
            result.Add(row);
        }
    ...
    public DataMap TechCost(string id)
    {
        ...
        if (def.Count == 0)
            return new();
        var result = ScaledCost(def.Map("cost"), Math.Pow(LegacyTechIds.Contains(id) ? 1.65 : 1.32, Tech.N(id)));
    ...
    public int FactoryProductionLanes(string kind, long site = -1) => HasResearch("I_G2") ? 2 : 1;
