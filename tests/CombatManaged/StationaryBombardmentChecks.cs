...
internal static class StationaryBombardmentChecks
{
    ...
    private static void SavedHoldAndReapproach()
    {
        ...
        // Derive a compact radius-8 legacy-form snapshot of this stopped actor only.
        b.HostileShots.Clear();b.Bursts.Clear();b.DamageNumbers.Clear();var r8=b.SerializeCombatSnapshot();CombatSnapshotCodec.TryDecode(r8.Value("payload"),out var data);var old=(DataMap)data!;var savedEnemy=old.List("enemies").OfType<DataMap>().Single();
        foreach(string key in new[]{"space_position","target_space"}){var v=savedEnemy.Vector3(key);savedEnemy[key]=v.Normalized()*(v.Length()-8f);}foreach(string key in new[]{"spawn_radius","combat_entry_radius"})if(savedEnemy.ContainsKey(key))savedEnemy[key]=savedEnemy.N(key)-8;r8["earth_radius"]=8d;r8["payload"]=CombatSnapshotCodec.Encode(old);
        var migrated=new Battlefield(g,new Surface());Check(migrated.RestoreCombatSnapshot(r8),"stationary actor migrates from explicit radius8 without an extra anchor field");var me=migrated.Enemies.Single();Near(me.Vector3("space_position").DistanceTo(e.Vector3("space_position")),0,"radius migration preserves stopped world height",.00001);migrated.Step(1d/60);Check(me.B("stationary_bombard")&&me.Vector3("space_position").Length()>CombatScale.EarthCollisionRadius,"migrated parked enemy stays safe and stationary");
    }
