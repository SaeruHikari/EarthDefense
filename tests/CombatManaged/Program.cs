...
foreach(string kind in new[]{"scout","carrier"})
{
...
foreach(long wave in new long[]{1,5,10,20,28,60,120})
{
    var plan=DefenseWavePlan.Build(wave,36,30,45,wave>=31?1:0,wave>=31?2:0);
...
UniformEnemyChecks.Run(Check);
LootPickupChecks.Run(Check);
