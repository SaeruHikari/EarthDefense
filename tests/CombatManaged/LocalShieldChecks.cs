...
internal static class LocalShieldChecks
{
    ...
    private static void ProjectilesAndMeteors()
    {
        ...
        var(mg,ms,mb)=Scene();var mt=Tower(mb);var meteor=mb.SpawnEnemy("meteor",new(){["wave"]=1L},Vector3.Back*(float)(mt.N("shield_radius")+.2))!;meteor["velocity"]=Vector3.Forward*6;mb.Active=true;double hp=mt.N("hp");mb.Step(.06);Check(!mb.Enemies.Contains(meteor),"meteor is stopped by a covered shield crossing");Near(mt.N("hp"),hp-3,"meteor impact consumes its original damage budget",1e-9);Near(mg.EarthHp,100,"shield intercepted meteor never impacts planet",1e-9);
    }
    private static void RepairRegenerationAndResearch()
    {
        ...
        a["hp"]=a.N("max_hp")*.4;double fraction=.4;g.Tech["shield"]=2L;g.InvalidateFactoryStats();a=Tower(b);Near(a.N("max_hp"),g.ShieldMax(),"historic shield investment increases each tower capacity",1e-9);Near(a.N("hp")/a.N("max_hp"),fraction,"capacity changes preserve the tower damage fraction",1e-9);Near(a.N("regeneration"),g.ShieldRegeneration(),"historic regeneration investment remains active",1e-9);
    ...
    private static void RepeatedQueryCache()
    {
        ...
        g.Tech["shield"]=1L;g.InvalidateFactoryStats();b.GetLocalShieldSummary();Check(s.TransformReads==first+2,"stat revision invalidates same-tick shield cache");
    ...
    {
        var fixture=DataMap.Parse(File.ReadAllText("tests/CombatManaged/Fixtures/combat-radius8.json"));var g=new DefenseState();Check(g.Restore(fixture.Map("game")),"real radius8 old game investment restores");double oldShield=g.Shield;var b=new Battlefield(g,new Surface());Check(b.RestoreCombatSnapshot(fixture.Map("battle")),"battle version3 restores as an empty local-shield ledger");Check(b.GetLocalShieldSummary().I("count")==0,"old game receives no free shield towers");double hp=g.EarthHp;Hit(b,Vector3.Back,1);Near(g.EarthHp,hp-1,"old global balance no longer grants protection without a tower",1e-9);Near(g.Shield,oldShield,"old balance retained solely for save compatibility",1e-9);
