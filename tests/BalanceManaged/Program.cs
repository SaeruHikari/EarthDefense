...
{
    Check(DataMap.Equivalent(DefenseWavePlan.Composition(wave),LegacyWavePlan127.Composition(wave)),"wave weights "+wave);
    ...
    foreach(int index in new[]{0,12,13,25})
    {
        ...
        var expected=new DataMap{["kind"]=kind};var actual=expected.DeepClone();
        LegacyEnemyCatalog127.Apply(expected,entry,state.CombatSettings);EnemyCatalog.Apply(actual,entry,state.CombatSettings);
        Check(DataMap.Equivalent(expected,actual),$"enemy exact {wave}/{stage}/{role}/{kind}/{index}");
    }
    foreach(long total in new long[]{1,4,30,128,1000})
    foreach(bool medium in new[]{false,true})
    {
        var old=LegacyWavePlan127.Build(wave,total,30,45,medium,2,3);
        var current=DefenseWavePlan.Build(wave,total,30,45,medium,2,3);
        Check(DataMap.Equivalent(old,current),$"full wave plan {wave}/{total}/{medium}");
        for(long i=0;i<current.L("planned_count");i++)
            Check(DataMap.Equivalent(LegacyWavePlan127.Entry(old,i),DefenseWavePlan.Entry(current,i)),"spawn order "+wave+"/"+i);
    }
}
var oldDirector=new LegacyInvasionDirector127();var director=new InvasionDirector();
oldDirector.ConfigureAnchor(new Vector3(.2f,.3f,.8f));director.ConfigureAnchor(new Vector3(.2f,.3f,.8f));
foreach(long wave in Enumerable.Range(1,65).Select(i=>(long)i))
{
    Check(DataMap.Equivalent(oldDirector.FrontsForWave(wave),director.FrontsForWave(wave)),"front positions "+wave);
    var a=new CombatRandom(127); var b=new CombatRandom(127);
    for(int i=0;i<20;i++)Check(DataMap.Equivalent(oldDirector.SpawnPoint(wave,a),director.SpawnPoint(wave,b)),"exact scatter "+wave+"/"+i);
    Check(oldDirector.WaveBudget(wave,10,2,128)==director.WaveBudget(wave,10,2,128),"count curve128 "+wave);
}
