...
public sealed class LegacyInvasionDirector127
{
    public const double MothershipRadius = CombatScale.EarthRadius + 5.65;
    public static readonly long[] UnlockWaves = { 1, 5, 9, 13, 17, 21, 25, 29 };
    private static readonly string[] Names = { "先锋航道", "晨星航道", "暮影航道", "极光航道", "寒夜航道", "赤潮航道", "深渊航道", "终环航道" };
    private static readonly Color[] Colors = { new("ff9477"), new("efbf76"), new("cf91e5"), new("7fbbea"), new("7cd4bd"), new("ef788c"), new("929fea"), new("f2d9ac") };
    public static readonly Vector3[] LocalDirections = { new(0, 0, 1), new(.82f, .25f, .52f), new(-.76f, -.35f, .55f), new(.25f, .95f, -.18f), new(-.22f, -.94f, -.25f), new(.82f, -.18f, -.55f), new(-.82f, .22f, -.53f), new(0, 0, -1) };
    ...
    public double SpawnConeRadians(long wave)
    {
        ...
        foreach (long threshold in UnlockWaves)
            if (threshold <= Math.Max(wave, 1))
                unlocked++;
        return C.Lerp(.20943951024, .31415926536, Math.Max(0, unlocked - 1) / 7d);
    }
    public DataMap SpawnPoint(long wave, CombatRandom rng)
    {
        ...
        for (int attempt = 0; attempt < 32; attempt++)
        {
            ...
            if (candidate.DistanceTo(mother) < 1.40)
                continue;
            position = candidate;
            break;
        }
    ...
    public Vector3 FrontierAircraftSpawn(Vector3 origin, CombatRandom rng)
    {
        ...
        double angle = rng.Randf() * Math.Tau, lateral = Math.Sqrt(rng.Randf()) * 2.8 + .25;
        return origin + basis * C.Vec(Math.Cos(angle) * lateral, Math.Sin(angle) * lateral, -rng.Range(1.4, 3));
    }
}
