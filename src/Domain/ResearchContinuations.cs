...
public static partial class DeepTechnology
{
    ...
    public static DataMap SuccessorCost(string branch, int rank)
    {
        ...
        return new()
        {
            ["science"] = Math.Min(DefenseState.ResourceLimit, Math.Ceiling(DomainBalance.Successor(branch).N("science_base") * Math.Pow(DomainBalance.Successor(branch).N("science_growth"), rank - 1))),
            ["alien_points"] = (long)Math.Min(DefenseState.MaxExactInteger, Math.Ceiling(DomainBalance.Successor(branch).N("alien_base") * Math.Pow(DomainBalance.Successor(branch).N("alien_growth"), rank - 1)))
        };
    }
    private static DataMap SuccessorDefinition(string branch, int rank)
    {
        var rule=DomainBalance.Successor(branch);
        string parent = rank == 1 ? branch + "_G2" : SuccessorId(branch, rank - 1);
        return new()
        {
            ["id"] = SuccessorId(branch, rank), ["name"] = rule.S("display_name") + " \u8fdb\u9636 " + rank,
            ["branch"] = branch, ["size"] = "medium", ["importance"] = 2L, ["alien"] = true,
            ["max"] = 1L, ["ranks"] = 1L, ["tier"] = 6L,
            ["effects"] = new List<object?> { rule.S("display_name") + " +" + (rule.N("increment")*100).ToString("0.###",CultureInfo.InvariantCulture) + "%" },
            ["values"] = new DataMap { [rule.S("attribute")] = rule.N("increment") },
