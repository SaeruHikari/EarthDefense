    ...
    private static DataMap MigrateLegacyResearch(DataMap legacy)
    {
        ...
        foreach (var (branch, budget) in values)
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                double amount = DataMap.Number(DeepTechnology.Migration127SourceDefinition(id).Map("values").Values.First());
                int slot = i % 5;
        ...
        foreach (var (id, owned) in direct)
            if (owned)
                nodes[id] = 1L;
        return DeepTechnology.Migrate127(nodes, nodes.Keys.ToHashSet(), new DataMap(), legacy, 0);
    }
}
