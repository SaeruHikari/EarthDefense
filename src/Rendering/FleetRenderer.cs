...
public sealed partial class FleetRenderer
{
    ...
    private readonly Dictionary<string, Aabb> _prototypeBounds = new();
    ...
    public void Sync(string category, IReadOnlyList<DataMap> actors, float scale, double time, long excludedUid = -1)
    {
        foreach (var bucket in _buckets.Values)
            if (bucket.Category == category)
                bucket.Reset();
        if (_frameScaleRevision != CatalogData.Revision)
        {
            FrameScales = AirframeCatalog.Definitions.ToDictionary(row => row.S("id"), row => (float)row.N("scale_multiplier", 1));
            _frameScaleRevision = CatalogData.Revision;
        }
        bool shieldCategory = category is "drones" or "enemies";
        if (shieldCategory) UnitShields?.BeginCategory(category);
        var inverse = _surface.GlobalBasis.Inverse();
        PrepareRenderPoses(category, actors, scale, time, excludedUid, inverse);
        for (int i = 0; i < actors.Count; i++)
        {
            ref readonly var pose = ref _renderPoses[i];
            if (!pose.Valid) continue;
            var bucket = GetBucket(category, pose.Visual!, pose.InSpace, pose.Critical);
            bucket.Transforms.Add(pose.Transform);
            bucket.States.Add(pose.State);
            bucket.Min = bucket.Min.Min(pose.Transform.Origin);
            bucket.Max = bucket.Max.Max(pose.Transform.Origin);
            if (shieldCategory && UnitShields != null && pose.State.R > 0 && actors[i].N("hp", 1) > 0)
                UnitShields.Add(category, pose.InSpace, pose.Transform, GetPrototypeBounds(pose.Visual!), pose.State.R, category == "drones");
        }
        if (shieldCategory) UnitShields?.EndCategory(category);
        foreach (var bucket in _buckets.Values)
            if (bucket.Category == category)
                Upload(bucket);
    }
    ...
    private Bucket GetBucket(string category, string visual, bool inSpace, bool critical)
    {
        ...
        var parts = GetPrototypeParts(visual);
    ...
    private Part[] GetPrototypeParts(string visual)
    {
    ...
    public Aabb GetPrototypeBounds(string visual)
    {
        if (_prototypeBounds.TryGetValue(visual, out var cached)) return cached;
        bool found = false;
        Aabb bounds = default;
        foreach (var part in GetPrototypeParts(visual))
        {
            // Animated charge lamps, exhaust and the retired energy shell are not hull geometry.
            if (part.State || part.Override is ShaderMaterial) continue;
            var current = part.Local * part.Mesh.GetAabb();
            bounds = found ? bounds.Merge(current) : current;
            found = true;
        }
        if (!found)
            foreach (var part in GetPrototypeParts(visual))
        ...
        return _prototypeBounds[visual] = bounds;
    }
    private static bool IsRetiredEnergyShell(Material? material) => material is ShaderMaterial shader
        && shader.HasMeta("combat_state") && shader.GetShaderParameter("surface_mode").AsDouble() < .5;
    private static void Collect(Node3D node, Transform3D parent, List<Part> parts)
    {
        ...
        if (node is MeshInstance3D mesh && mesh.Mesh != null && mesh.Mesh.GetSurfaceCount() > 0 && mesh.Visible)
        {
            ...
            bool oldShell = IsRetiredEnergyShell(mesh.MaterialOverride);
            if (mesh.MaterialOverride == null)
            {
                oldShell = true;
                for (int i = 0; i < mesh.Mesh.GetSurfaceCount(); i++)
                    oldShell &= IsRetiredEnergyShell(mesh.Mesh.SurfaceGetMaterial(i));
            }
            if (!oldShell) parts.Add(new(mesh.Mesh, mesh.MaterialOverride, local, state));
        }
