...
public sealed partial class Battlefield
{
    ...
    public DataMap FleetCapacityState()
    {
        ...
        var occupancy = new Dictionary<long, HashSet<int>>();
        foreach (var drone in Drones)
        {
            int cost = C.I(drone, "capacity_cost", 1), berth = C.I(drone, "patrol_slot");
            used += cost; long site = C.L(drone, "factory_site_id");
            if (!occupancy.TryGetValue(site, out var occupied)) occupancy[site] = occupied = new();
            for (int point = 0; point < cost; point++) occupied.Add(berth + point);
        }
        foreach (var f in Factories.Values)
        {
            capacity += C.I(f, "capacity");
            if (!occupancy.TryGetValue(C.L(f, "site_id"), out var occupied)) occupied = new();
