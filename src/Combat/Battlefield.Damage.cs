...
public sealed partial class Battlefield
{
    ...
    public void DetonateDroneDeath(Vector3 at, DataMap? stats = null, DataMap? source = null)
    {
        ...
        var context = C.Shallow(source ?? C.Empty);
    ...
    public void DetonateMissile(Vector3 at, double damage, double radius = -1, DataMap? context = null)
    {
        ...
        double blastMultiplier = Math.Max(0, C.N(context, "missile_blast_damage_multiplier", 1));
        if (blastMultiplier != 1)
        {
            damage *= blastMultiplier;
            // The split delayed half already owns its amplified budget; child missiles retain their own unamplified parent budget.
            context["missile_blast_damage_multiplier"] = 1d;
        }
    ...
    public double ApplyEnemyDamage(DataMap e, double damage, Color? color = null, DataMap? context = null)
    {
        ...
        long damageMark = Performance?.Timestamp() ?? 0;
        var packet = context;
        ...
        if (Performance != null) damageMark = Performance.Record(CombatStage.DamageArmor, damageMark);
        if (result.Total <= 0)
        {
            e["shield_flash"] = .12;
            return 0;
        ...
        if (Performance != null) damageMark = Performance.Record(CombatStage.DamageFeedback, damageMark);
        ...
        Performance?.Record(CombatStage.DamageAfterEffects, damageMark);
        return result.Total;
    }
    ...
    private double DamageAbilityBonus(DataMap e, double damage, string type, DataMap source)
    {
        ...
        if (type == "beam" && primary && EnemyArmor.Layer(e) == "energy" && C.B(tech, "L_A3") && C.Packet(source, "laser_first_energy_multiplier", 1) != 1 && actor != null && Clock >= C.N(actor, "first_energy_ready") && C.L(actor, "first_energy_uid", -1) != C.L(e, "uid"))
        {
            damage *= C.Packet(source, "laser_first_energy_multiplier", 1);
    ...
    private static readonly (string Prefix, string Key)[] PerkStatusFields = { ("perk_slow", "perk_slow_remaining"), ("perk_erosion", "perk_erosion_remaining") };
    private void UpdatePerkStatus(DataMap e, double dt)
    {
        foreach (var (prefix, key) in PerkStatusFields)
        {
    ...
    private void AddBeam(Vector3 origin, Vector3 destination, Color color, bool laser = false)
    {
        if (Beams.Count >= 256) return;
        var beam = new DataMap
        {
            ["from_space"] = origin, ["to_space"] = destination,
            ["life"] = .17, ["max_life"] = .17, ["color"] = color
        };
        if (laser) beam["style"] = "laser";
        Beams.Add(beam);
    }
