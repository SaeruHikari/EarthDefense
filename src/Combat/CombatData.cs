...
public interface ICombatSurface
{
    ...
    bool TryGetCoordinateFrame(out Transform3D localToWorld) { localToWorld = Transform3D.Identity; return false; }
}
...
internal static class C
{
    ...
    public static double N(DataMap? m, string k, double d = 0) => m != null && m.TryGetValue(k, out var value) ? value is double number ? number : DataMap.Number(value, d) : d;
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static long L(DataMap? m, string k, long d = 0) => m != null && m.TryGetValue(k, out var value) ? value is long number ? number : DataMap.Integer(value, d) : d;
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static int I(DataMap? m, string k, int d = 0) => m != null && m.TryGetValue(k, out var value) ? value is int number ? number : (int)Math.Clamp(DataMap.Integer(value, d), int.MinValue, int.MaxValue) : d;
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static string S(DataMap? m, string k, string d = "") => m != null && m.TryGetValue(k, out var value) && value is string text ? text : d;
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool B(DataMap? m, string k, bool d = false) => m != null && m.TryGetValue(k, out var value) && value is bool flag ? flag : d;
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    public static DataMap Shallow(DataMap m) => new(m);
