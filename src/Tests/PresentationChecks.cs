...
public partial class PresentationChecks : Node
{
    ...
    public override async void _Ready()
    {
        try
        {
            ...
            string perk = ui.VisibleIds.First(id => PerkCatalog.Definition(id).I("stage") == 0);
            Check(!_app.Game.FactoryPerks.IsUnlocked(perk), "fresh inventory shows unowned perks");
            ui.SelectPerk(perk);
            ui.Refresh();
            await Frames();
            Check(ui.UpgradeButton.Disabled && ui.UpgradeButton.Text.Contains("购买"), "unowned perk exposes disabled purchase while chips are insufficient");
            Check(_app.Game.FactoryPerks.CreditAlienChips(500), "native shop fixture receives actual alien-chip currency");
            ui.Refresh();
            await Frames();
            long purchasePrice = _app.Game.FactoryPerks.PurchaseCost(perk), shopChips = _app.Game.FactoryPerks.AlienChips;
            Check(!ui.UpgradeButton.Disabled && ui.UpgradeButton.Text.Contains("购买"), "funded unowned perk offers purchase in existing detail action");
            await Capture("perk-chip-purchase-ui");
            await Click(ui.UpgradeButton.GetGlobalRect().GetCenter());
            Check(_app.Game.FactoryPerks.GetLevel(perk) == 1 && _app.Game.FactoryPerks.AlienChips == shopChips - purchasePrice, "native shop purchase creates level one and charges exact chip price");
            Check(_app.Game.FactoryPerks.SlotsFor("interceptor")[0] == "" && ui.UpgradeButton.Text.Contains("升级"), "purchase preserves empty slot and action becomes upgrade");
            Check(!_app.Game.FactoryPerks.Purchase(perk) && _app.Game.FactoryPerks.AlienChips == shopChips - purchasePrice, "duplicate purchase cannot charge existing native inventory item");
            var tile = ui.InventoryTiles[perk];
            var target = ui.SlotButtons[0];
            long chips = _app.Game.FactoryPerks.AlienChips;
            ...
            Check(_app.Game.FactoryPerks.AlienChips == chips, "equipment does not spend purchase or upgrade chips");
            ...
            Check(_app.Game.FactoryPerks.AlienChips == chips - price, "native upgrade spends exact chip price");
            Check(_app.Game.FactoryPerks.UpgradeCost(perk) > price, "following upgrade price increases along the chip curve");
            string advanced = ui.VisibleIds.First(id => PerkCatalog.Definition(id).I("stage") > 0);
            ui.SelectPerk(advanced);
            await Frames();
            Check(ui.UpgradeButton.Disabled && !_app.Game.FactoryPerks.IsUnlocked(advanced), "advanced purchase remains locked before real frontier milestone");
