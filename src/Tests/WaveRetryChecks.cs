...
public partial class WaveRetryChecks : Node
{
    ...
    public override async void _Ready()
    {
        try
        {
            ...
            var reward = _app.Game.FactoryPerks.ClaimAlienChip(_app.Game.RunId, "earth:wave:1:enemy:3");
            Check(reward.B("claimed"), "fixture earns a permanent aircraft chip after opening");
            long permanent = _app.Game.FactoryPerks.AlienChips;
            string profileHash = Digest("earthward_alien_chip_perks.json");
            await DefeatAndClickRetry();
            AssertWaveStart(opening, "native defeat retry");
            Check(_app.Game.FactoryPerks.AlienChips == permanent && Digest("earthward_alien_chip_perks.json") == profileHash, "retry never writes back an older permanent perk profile");
            Check(_app.Game.FactoryPerks.ClaimAlienChip(_app.Game.RunId, "earth:wave:1:enemy:3").B("duplicate") && _app.Game.FactoryPerks.AlienChips == permanent, "replaying same aircraft death cannot duplicate permanent chip");
