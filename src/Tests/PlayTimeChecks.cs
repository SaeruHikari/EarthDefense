...
public partial class PlayTimeChecks : Node
{
    ...
    public override async void _Ready()
    {
        try
        {
            ...
            var legacy = DataMap.Parse(System.IO.File.ReadAllText(SaveFile));
            legacy.Remove("play_time_seconds"); legacy.Remove("play_time_estimated");
            legacy.Map("game")["defense_time"] = 1234.5d;
            Check(_app.ValidateCheckpoint(legacy), "checkpoint without new timer fields remains compatible");
            _app.FlushCheckpointWrites();
            System.IO.File.WriteAllText(SaveFile, legacy.ToJson());
            Check(_app.LoadCheckpoint(), "real load accepts old checkpoint without timer metadata");
            Check(Near(_app.PlayTimeSeconds, 1234.5) && _app.PlayTimeEstimated, "old elapsed defense time provides explicit estimate");
            Check(_app.PlayTimeText == "≈00:20:34", "estimated legacy duration is visibly marked with approximate symbol");
            _app.UserPaused = false; _app.Speed = 2; Step(.5);
            Check(Near(_app.PlayTimeSeconds, 1235) && _app.PlayTimeEstimated, "new exact seconds extend historical estimate without losing provenance");
            Check(_app.SaveCheckpoint(), "estimated provenance saves successfully");
            var estimated = DataMap.Parse(System.IO.File.ReadAllText(SaveFile));
            Check(estimated.B("play_time_estimated") && Near(estimated.N("play_time_seconds"), 1235), "saved estimate is not silently relabeled exact");
            Check(_app.LoadCheckpoint() && _app.PlayTimeEstimated && Near(_app.PlayTimeSeconds, 1235), "reload preserves estimated clock rather than recomputing defense time");
            var invalid = estimated.DeepClone(); invalid["play_time_seconds"] = -1d;
